...
class Cache(object):
    ...
    def get_digest(self, private_id, public_id=None):
        ...
        if columns[DIGEST_HA1] and columns[public_id_column]:
    ...
    def put_digest(self, digest, private_id):
        pass

    def get_IMSSubscription(self, public_id, private_id=None):
        pass

    def put_IMSSubscription(self, public_id, private_id=None):
        pass

    def get_iFC(self, public_id, private_id=None):
        pass

    def put_iFC(self, public_id, private_id=None):
        pass
...
class DigestHandler(BaseHandler):
    def get(self, private_id):
       ...
       retval = yield singleton_cache.get_digest(private_id, public_id)
       self.send_error_or_response(retval)
       
    def put(self, private_id):
       try:
           digest = json.dumps(self.request.body)['digest_ha1']
           retval = yield singleton_cache.put_digest(digest, private_id)
           self.send_error_or_response(retval)
       except:
