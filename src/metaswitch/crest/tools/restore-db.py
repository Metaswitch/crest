...
"Restores the gzipped CSV files given on the command line (created with backup-db.py) into the Cassandra database. Invoked with './restore-db.py file1.csv.gz file2.csv.gz ...' "
...
def do_this():
        for gzfilename in sys.argv[1:]:
                ...
                print "Successfully restored %d records from %s" % (n, gzfilename)
                f.close()
        reactor.stop()
