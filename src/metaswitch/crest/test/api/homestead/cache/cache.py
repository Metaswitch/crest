...
class TestCache(unittest.TestCase):
    ...
    def test_put_digest(self):
        self.cass_client.batch_insert.return_value = defer.Deferred()
        self.cache.put_digest("priv", "digest")
    ...
    def test_put_associated_public_id(self):
        self.cass_client.batch_insert.return_value = defer.Deferred()
        self.cache.put_associated_public_id("priv", "kermit")
        self.cass_client.batch_insert.assert_called_once_with(
                                             key="priv",
                                             column_family="impi",
    ...
    def test_put_ims_subscription(self):
        self.cass_client.batch_insert.return_value = defer.Deferred()
        self.cache.put_ims_subscription("pub", "xml")
        self.cass_client.batch_insert.assert_called_once_with(
                                        key="pub",
                                        column_family="impu",
    ...
    def test_get_ims_subscription(self):
        self.cass_client.get_slice.return_value = defer.Deferred()

        get_deferred = self.cache.get_ims_subscription("pub")
        get_cb = mock.MagicMock()
        get_deferred.addCallback(get_cb)

        self.cass_client.get_slice.assert_called_once_with(
                                                 key="pub",
                                                 column_family="impu",
                                                 names=["ims_subscription_xml"])
        self.cass_client.get_slice.return_value.callback(
                               [self.MockColumn("ims_subscription_xml", "xml")])
        self.assertEquals(get_cb.call_args[0][0], "xml")

    def test_get_digest_no_pub_id_supp(self):
        self.cass_client.get_slice.return_value = defer.Deferred()

        get_deferred = self.cache.get_digest("priv")
        get_cb = mock.MagicMock()
        get_deferred.addCallback(get_cb)
        ...
        self.cass_client.get_slice.assert_called_once_with(
                                                 ...
                                                 names=["digest_ha1"])
        self.cass_client.get_slice.return_value.callback(
                                      [self.MockColumn("digest_ha1", "digest")])
        self.assertEquals(get_cb.call_args[0][0], "digest")

    def test_get_digest_no_pub_id_assoc(self):
        self.cass_client.get_slice.return_value = defer.Deferred()

        get_deferred = self.cache.get_digest("priv", "miss_piggy")
        get_cb = mock.MagicMock()
        get_deferred.addCallback(get_cb)
        ...
        self.cass_client.get_slice.return_value.callback(
                                    [self.MockColumn("digest_ha1", "digest"),
                                     self.MockColumn("public_id_kermit", None)])
        self.assertEquals(get_cb.call_args[0][0], None)
    ...
    def test_get_digest_right_pub_id(self):
        ...
        self.cass_client.get_slice.return_value.callback(
                                [self.MockColumn("digest_ha1", "digest"),
                                 self.MockColumn("public_id_miss_piggy", None)])
        self.assertEquals(get_cb.call_args[0][0], "digest")
